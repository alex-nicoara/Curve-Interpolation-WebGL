...
/////////////////////////////////////////////////////////////////////////////////////////
//
// cs411 assignment 3 (Fall 2017) - spline curves
//
/////////////////////////////////////////////////////////////////////////////////////////
...
function tensionUp() {
  ...
  var ct = ctrlPts.length/2;
  for(var i = 1; i <= ct-3; i++) {
    if(ct > 3) {
      var pk_n1 = {x: ctrlPts[2*i-2], y: ctrlPts[2*i-1]}; // p_{k-1}
      var pk    = {x: ctrlPts[2*i], y: ctrlPts[2*i+1]}; // p_{k}
      var pk_p1 = {x: ctrlPts[2*i+2], y: ctrlPts[2*i+3]}; // p_{k+1}
      var pk_p2 = {x: ctrlPts[2*i+4], y: ctrlPts[2*i+4]}; // p_{k+2}
...
function interpolate(pk_n1,pk,pk_p1,pk_p2)
{
  ...
  for(var u=0; u<=1; u+=uStep) {
    var x1 = pk_n1.x, x2 = pk.x, x3 = pk_p1.x, x4 = pk_p2.x;
    var y1 = pk_n1.y, y2 = pk.y, y3 = pk_p1.y, y4 = pk_p2.y;

    /* Calculate x */
    var a_x = (-t*x1)+((2-t)*x2)+((t-2)*x3)+(t*x4);
    var b_x = (2*t*x1)+((t-3)*x2)+((3-(2*t))*x3)+(-t*x4);
    var c_x = (-t*x1)+(t*x3);
    var d_x = x2;

    var finalx = (Math.pow(u,3)*a_x)+(Math.pow(u,2)*b_x)+(u*c_x)+(1*d_x);

    /* Calculate y */
    var a_y = (-t*y1)+((2-t)*y2)+((t-2)*y3)+(t*y4);
    var b_y = (2*t*y1)+((t-3)*y2)+((3-(2*t))*y3)+(-t*y4);
    var c_y = (-t*y1)+(t*y3);
    var d_y = y2;

    var finaly = (Math.pow(u,3)*a_y)+(Math.pow(u,2)*b_y)+(u*c_y)+(1*d_y);
